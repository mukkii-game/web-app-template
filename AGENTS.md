# AGENTS.md

> 全 AI 共通の作法。Claude は CLAUDE.md の `@AGENTS.md` で、Gemini は設定(contextFileName)でこのファイルを読む。Phaser の API は `docs/phaser/reference/` を題目ごとに引く(入口は `phaser4` スキル)。

## これは何か
- Mukkii の個人開発の作品 1 本。パイロット版(遊べるモック)を最短で公開するための repo。
- 企画は `SPEC.md`。読んでから始めろ。冒頭に「追加で読むルール」があればそれも読め。書いてないことは自分で決めろ。

## 進め方(最重要)
1. **止まるな。** 伝えられたゴールまで、だめもとで作り切れ。途中で確認を取りに来るな。
2. **ゴールは改造の土台。** 「◯◯みたいなの」は、そっくりを仕上げるのではなく、人間がそこから改造できる状態まで作ること。
3. **質問は溜めろ。** 迷ったら仮決めして進み、`QUESTIONS.md` に「何を仮決めしたか、他の選択肢」を並べろ。人間は後でまとめて答える。
4. **複数案は `variants/a`, `variants/b` …** に、それぞれ動く状態で置け。
5. **パイロットは製品ではない。** セキュリティ監査、網羅テスト、文書の肥大化はするな。動いて公開できればよい。
6. 終わったら `HANDOFF.md` を更新しろ(現状、次の一手、既知の問題。別の AI が引き継ぐ前提で)。

## 知らないと必ず溶かす(実機で確認済み)
- **入力リスナーを `window` だけに付けるな。** itch.io のモバイル埋め込みでは `window` に
  イベントが 1 つも届かないことがある。canvas / body / document など複数に付けて重複排除する。
- **「直したのに直らない」が 2 回続いたら、修正ではなくキャッシュを疑え。**
  画面の隅に BUILD ID を出し、`version.json` と照合して自動リロードする仕組みを最初に入れる。
- **スマホにはコンソールが無い。** `?debug=1` で画面に生イベント数を出す手段を先に作れ。
  それが無い間の修正は全部あてずっぽうになる。
- 詰まったら `web-game-trouble` スキル(`docs/knowledge/`)。§8 が症状 → 原因の逆引き表。

## 技術の既定値
- Phaser 4 + Vite + TypeScript。小さいものは素の canvas / DOM でもよい。
- 3D は動きの激しいものは Babylon.js、見せるのが主なら Three.js。
- `src/core/`(save, i18n, audio, input, demo, meta)を先に使え。同じものを作り直すな。
- 型で止まるな。動く方を優先。ビルド出力は `dist/`、Vite の `base` は `./`(深い URL や iframe でも動く)。
- `?auto=1` で自動プレイが走るようにしろ(`core/demo.ts` に update を実装)。CI の確認と宣伝動画に使う。
- `?lang=en` で英語になるようにしろ。文字列は `i18n` を通せ。

## 素材
- **速さを取れ。** それらしいものが早く埋まる方を選ぶ。素材は外部(CC0 等)でも AI 生成でもよく、出所が明確なら手段は問わない。止まって聞くな。
- `assets/mine/` に人間が置いた素材があれば、それを最優先で使う(自分で描いたもの)。
- 使った素材は全部 `CREDITS.md` に出典・規約・クレジット文・(AI 生成なら)どの AI かを書け。書いてない素材は使うな。

## 公開
- `docs/PUBLISH.md` を日本語と英語で埋めてから公開しろ(タイトル、一言、説明、操作、タグ)。
- main に push すれば Pages に出る。itch.io は `publish-itch` の手動ボタン。
- `.github/` も必要なら変えてよい。変えたら理由を `QUESTIONS.md` に 1 行残せ。

## 文書
- `SPEC.md` の挙動を変えたら `SPEC.md` を直せ。長くするな。
- 設計判断は `DECISIONS.md` に短く追記(過去の行は書き換えない。後から経緯を追うため)。

## 任せる範囲(ここは確認不要)
- **この repo の中は全部あなたのものだ。** 追加・変更・削除・依存の追加・CI の調整・実行まで、確認を取らずに最後までやれ。壊しても git で戻せる。
- 外に出る操作(公開、外部サービスへの送信)だけは人間が押す。作るところまでは止まるな。

## 禁止
- テストを消す・スキップする。ライセンス不明の素材。秘密情報のコミット。
- 同じ作品 repo に同時に push する経路を 2 つ作る(サブエージェントや他セッションへの質問は自由。push する者は 1 つ)。
- 会社・仕事関係のフォルダやドライブへの書き込み(この repo の外は触らない)。

## ローカル
```
npm ci && npm run dev      # 開発
npm run build              # dist/
node tools/check.mjs       # 起動確認 + スクショ
```
